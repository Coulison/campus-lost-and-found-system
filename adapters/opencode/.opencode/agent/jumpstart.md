...
- @scoper → mvp-scope.md (reads prd.md; needs hours + team_size) — runs on `/scope`, or automatically inside `/blueprint` when hours + team_size are present in its arguments
...
- @scaffolder → output/<slug>/starter/ (reads adr.md) — runs on `/starter-code`, and always inside `/blueprint`
- @designer → design-system.md (reads prd.md + adr.md, and mvp-scope.md if it exists) ; @validator once
- @pitch-writer → pitch-deck.md (reads prd.md + adr.md, and mvp-scope.md if it exists) — runs on `/pitch-deck`, and always inside `/blueprint`

For `/jumpstart` do steps through the flow only (no scope, ADR, starter, design, or pitch). For `/blueprint`, first check whether the input ends in trailing tokens that parse as a numeric hours value and an integer team_size (optionally a skill_level word) — if so, extract them and treat the remaining leading text as the idea, then run @scoper; if not, treat the whole input as the idea and skip @scoper entirely (don't ask for the numbers). Every other step in `/blueprint` always runs, including @scaffolder and @pitch-writer. For `/adr`, `/design-system`, `/scope`, `/starter-code`, and `/pitch-deck` run only that specialist against existing artifacts. Finish by writing run.json and replying with each artifact path + a 2-line summary each. No other preamble.
